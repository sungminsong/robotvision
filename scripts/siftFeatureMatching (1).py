...
cap = cv2.VideoCapture('../data/cup.mp4',cv2.IMREAD_GRAYSCALE)
suc, img1 = cap.read()
x,y,w,h	= cv2.selectROI('img', img1, False)
if w and h:
    roi = img1[y:y+h, x:x+w]
    cv2.imshow('car', roi)  # ROI 지정 영역을 새창으로 표시
    cv2.moveWindow('car', 0, 0) # 새창을 화면 좌측 상단에 이동
    cv2.imwrite('./car.jpg', roi)   # ROI 영역만 파일로 저장
    img2 = roi.copy()
